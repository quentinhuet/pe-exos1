...
couleurs = fichier.readlines()
for i in couleurs :
    sep = i.split()
    dico[sep[0]] = [int(sep[1]), int(sep[2]), int(sep[3])]
...
def patchwork(coul):
    n = len(coul)
    dim = int(np.sqrt(n))+1
    rep = np.zeros((dim,dim,3))
    rep[:]=255
    ligne = 0
    colonne = 0
    for part in coul:
        rep[ligne,colonne]=dico[part]
        if colonne == dim-1:
            colonne = 0
            ligne += 1
        else:
...
# %%
liste=[i[0] for i in dico.items() if (255 in i[1])]
print(liste)
...
# %%
mines = plt.imread('les-mines.jpg')
plt.imshow(mines)
